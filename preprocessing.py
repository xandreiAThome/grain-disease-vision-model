...
""" Resize the aspect ratio to 1:1 by padding to the longest side """
...
def create_validation_split(
    grain_type, val_ratio=0.15, random_state=42, restore_original=False
):
    """
    ...
    for category in categories:
        ...
        train_imgs, val_imgs = train_test_split(
            images, test_size=val_size, random_state=random_state
        )
        ...
        print(
            f"  {category}: {len(train_imgs)} train, {len(val_imgs)} val (from {len(images)} total)"
        )
    ...
    if split == "train":
        return A.Compose(
            [
                ...
                ),  # shifts the hue, saturation and brightness. Bc irl the pictures will be taken in diff lighting conditions
                A.GaussNoise(
                    var_limit=(10.0, 30.0), p=0.2
                ),  # gaussion noise is like random pixels to match imperfections that can happen irl
...
def augment_minority_classes(grain_type, target_count=None, random_state=42):
    ...
    if already_augmented:
        ...
        print(
            f"To re-run, manually delete files with the aug_ prefix from train/ first.\n"
        )
        return {}
    ...
    for category in categories:
        ...
        if current_count >= target_count:
            print(
                f"  {category}: Already has {current_count} images, no augmentation needed."
            )
            generated_counts[category] = 0
            continue
